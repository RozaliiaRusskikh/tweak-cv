...
| 3 | `section` | Score display: `keyword_coverage`, `no_hallucination`, `quality_judge` (if available) | Always |
...
When `iteration >= 4`, the Edit button is removed and `notify_node` posts a thread warning:
> ⚠️ Maximum edits reached – please approve or reject the current version
