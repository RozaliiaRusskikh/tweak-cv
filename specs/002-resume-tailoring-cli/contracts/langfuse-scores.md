...
One Langfuse trace is created per CLI invocation in `main.py`:

```python
trace = langfuse_client.trace(
    name="resume-tailoring",
    input={"jd_text": jd_text, "company": company, "role": role},
)
```

`trace.id` is stored in `TailorState["langfuse_trace_id"]` and `jobs.langfuse_trace_id` (DB). All nodes reference it by ID — they do not hold a reference to the trace object.
...
| `quality-judge` | Quality Judge | `gemini-2.5-flash` | `nodes/score.py` |
